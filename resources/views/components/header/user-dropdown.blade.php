<div class="relative" x-data="{
    ...
    <button class="flex items-center text-gray-700 dark:text-gray-400" @click.prevent="toggleDropdown()" type="button">
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            ...
            {{-- จุดที่ 1: รูปภาพบน Navbar --}}
            @if (Session::has('employee_image_name') && Session::get('employee_image_name') !== null)
                <img src="{{ route('keycloak.employee.image', ['filename' => Session::get('employee_image_name')]) }}"
            ...
            @else
                {{-- กรณีไม่มีรูป ให้สร้างวงกลมแล้วใส่ตัวอักษรแรกของชื่อเข้าไปแทน --}}
                <div class="d-flex align-items-center justify-content-center bg-primary text-white rounded-circle"
                    ...
                    {{ strtoupper(substr(Session::get('employee_name_th') ?? 'User', 0, 1)) }}
                </div>
            @endif
        </span>
        ...
        <span
            class="block mr-1 font-medium text-theme-sm">{{ Session::get('employee_title_th') }}{{ Session::get('employee_name_th') }}</span>
    ...
    <div x-show="dropdownOpen" x-transition:enter="transition ease-out duration-100"
        ...
        <div>
            <span
                class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ Session::get('employee_title_en') }}{{ Session::get('employee_name_en') }}</span>
            <span
                class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ Session::get('employee_dss_mail') }}</span>
            <span
                class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ Session::get('employee_position') }}</span>
            <span
                class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ Session::get('org_div_division_name') }}</span>
        </div>
